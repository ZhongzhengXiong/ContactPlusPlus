...
const filters = {
    // search: (contacts, search) => contacts.filter(contact => contact.first_name.match(search)),
    all: contacts => contacts,
    tag: (contacts, tag) => contacts.filter(contact => contact.tag === tag)
}
...
export const getters = {
    filteredContacts: state => {
        const orderedContacts = _.orderBy(state.contacts, ['first_name', 'last_name'], ['desc'])
...
function saveHeadPortrait(src, contact_id) {
    // check whether directory exists, if not, then create  new directory
    let contactDir = jetpack.cwd(remote.app.getPath('userData'))
    console.log(`user directory is ${contactDir}`)
    contactDir.dir(`ContactPlusPlus/${contact_id}/head_portrait`)
    // now userDataDir is in userDataDir/ContactPlusPlus/some contact's id/head_portrait/
    console.log(contactDir.cwd())

    // rename the file
    const fileExtension = src.split('.').pop()
    const newFilename = `head_portrait.${fileExtension}`
    //copy to this contact's directory

    contactDir.copy(src, newFilename, {
        overwrite: (srcInspectData, destInspectData) => {
            return srcInspectData.modifyTime > destInspectData.modifyTime;
        }
    })
    //return new file path
    return contactDir.path(contactDir.path(newFilename))
}
...
export default{
    addContact(doc) {
        ...
        db.addContact(doc, newDoc => {
            console.log('INFO: successfully add document: ' + JSON.stringify(newDoc));
        })
        // sync with remote account
    },

    deleteContact(_id) {
        // delete all contact related material
        // add into local database
        db.deleteContact(_id)
        // sync with remote account
    },

    updateContact(_id, doc) {
        // update contacts' photo
        doc.head_portrait = saveHeadPortrait(doc.head_portrait, _id)
    ...
    fetchPageContacts(skip, limit, cb) {
        db.fetchPageContacts(skip, limit, cb)
    },

    /*
    sortby: by 'tag' or by firstname default
    */
    fetchPageContactsList(skip, limit, cb, sortby) {
        db.fetchContactListByPage(skip, limit, cb, sortby)
    }
}
